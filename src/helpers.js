import { TEST_IEX_API_KEY, IEX_API_KEY } from "./secrets.js";
...
export async function getStockQuote(symbol) {
  ...
  //   );
  const results = await http.get(
    `https://cloud.iexapis.com/stable/stock/${symbol}/quote?displayPercent=true&token=${IEX_API_KEY}`
  );
  return results;
}
...
export async function getMutipleStocks(symbols) {
  const results = await http.get(
    `https://cloud.iexapis.com/stable/stock/market/batch?symbols=${symbols}&types=quote&last=5&token=${IEX_API_KEY}`
  );
